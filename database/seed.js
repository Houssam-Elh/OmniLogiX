...
const Actor = require('../backend/models/Actor');
const Event = require('../backend/models/Event');
const FAQ = require('../backend/models/FAQ');
const Forum = require('../backend/models/Forum');
const Taxonomy = require('../backend/models/Taxonomy');
const Bourse = require('../backend/models/Bourse');
...
function cleanMongoJSON(data) {
  ...
  if (typeof data === 'object') {
    ...
    if (data.$oid && typeof data.$oid === 'string') {
      return data.$oid; // Return string id directly (Mongoose matches string as ObjectId automatically)
    }
    // Check if it is $date
    if (data.$date) {
...
async function seedDatabase() {
  try {
    ...
    const basePath = path.join(__dirname, '../backend/data/package-models-corrected');
